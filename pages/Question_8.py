...
def df_8():
    '''
    This function uses the football API for the venue information and the 
    openligadb API to construct a DataFrame that consists of the winrate 
    for every combination of team and venue capacity
    i: none
    o: DataFrame
    '''
    ...
    for x in range(3):
        ...
        for team_venue in response ['response']:
            ...
            venue_id = team_id_mapper [team_id]
            if venue_id not in venues.keys() and venue_id != None: 
    ...
    for x in range(15):
        ...
        for match in response:
            ...
            if venue_id in venues.keys() and team_id_2 in venues.keys():       
                team_name_2 = match['team2']['teamName']
                capacity = venues[venue_id]['capacity']
                if team_name_2 not in result.keys():        # new team
                    wins[team_name_2] = {}
                    total[team_name_2] = {}
                    result[team_name_2] = {}
                if capacity not in result[team_name_2].keys():      # new venue 
...
def upgrade_graph_8(team_chosen):
    '''
    This function computes the heatmap graph for the chosen teams.
    i: array of  teams
    o: updated graph
    '''
