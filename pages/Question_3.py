...
fig_Q3_r = px.scatter(df3_r, x="Rain in mm", y="Average goals per match", trendline="ols", hover_data="Sample size")
df3_p["Average goals per match"] = avgs_p
df3_p["Precipitation in mm"] = precs_p
df3_p["Sample size"] = ss_p
fig_Q3_p = px.scatter(df3_p, x="Precipitation in mm", y="Average goals per match", trendline="ols", hover_data="Sample size")

layout = [
        html.Div([
            html.H3('Question 3: How does high precipitation during a match influence the total number of goals scored?'),
            ...
            "a size of 6 or greater are considered to eliminate outliers. As we can see, the observed trend varies little when considering rain specifically " \
